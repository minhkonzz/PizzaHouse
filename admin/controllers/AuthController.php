   ...
   class AuthController extends Controller {
      ...
      public function handleAuthorizationCode(Request $req, array $params = []) {
         try {
            ...
            $_SESSION['okta_access_token'] = $access_token;

            $payloads = json_decode(base64_decode(explode('.', $access_token)[1]), true); 
            $req_sender->setUrl($_ENV["OKTA_DOMAIN"] . "/api/v1/users/" . $payloads["uid"]);
            $current_user = json_decode($req_sender->get(OKTA_API_REQUEST_HEADERS));
            if (nonnull($current_user)) $_SESSION["current_user"] = $current_user;
